...
// @version      0.1
// @description  Плавающая кнопка Kinomix слева от карточки фильма (запасной вариант)
...
(function () {
    ...
    const TARGET_SITE = 'https://kinomix.web.app';
    const PATH_TEMPLATE = '/film/{id}/';
    ...
    function createFloatingButton(filmId) {
        ...
        btn.onclick = (e) => {
            ...
            const url = TARGET_SITE + PATH_TEMPLATE.replace('{id}', filmId);
            window.open(url, '_blank');
        };

        document.body.appendChild(btn);
        console.log(`[Kinomix Floating] Плавающая кнопка добавлена для ID ${filmId}`);
    }
